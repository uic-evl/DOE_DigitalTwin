...
class RobotControl(BehaviorScript):
    def on_init(self):
        ...
        self.sock.bind("tcp://*:12348") #FOR GRIPPER
    ...
    def on_update(self, current_time: float, delta_time: float):
        ...
        # Get gripper attribute state
        switch = self.stage.GetPrimAtPath(str(self.prim_path)+ '/Switch')
        switch_state = switch.GetAttribute("JWM:Switch_state").Get()
    

        # Send joint positions as a single string
        try:
            self.sock.send_string(str(switch_state))
