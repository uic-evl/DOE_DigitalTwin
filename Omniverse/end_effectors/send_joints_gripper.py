...
class RobotControl(BehaviorScript):
    ...
    def on_update(self, current_time: float, delta_time: float):
        ...
        #logger.info(self.robot.dof_names)

        # Append gripper state
        switch_prim = self.stage.GetPrimAtPath(str(self.prim_path) + '/Switch')
        gripper_state = switch_prim.GetAttribute("JWM:Switch_state").Get()
        joints_str += str(gripper_state)    

        #logger.warn(joints_str)
