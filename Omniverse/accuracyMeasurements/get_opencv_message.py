...
class GetTagOrigin(BehaviorScript):
    ...
    def on_update(self, current_time: float, delta_time: float):
        try:
            ...
            # Format: x,y,z,rx,ry,rz;tx,ty,tz,trx,try,trz

            # camera: z-forward, x-right, y-down
            # ovis: x-forward, y left, z up

            data = message.split(";")

            # Camera pose from origin tag
            xform_data = data[0].split(",")

            logger.warn(xform_data)

            x = (float(xform_data[0])) 
            y = (float(xform_data[1])) - 0.13
            z = (float(xform_data[2])) 

            qx = float(xform_data[3]) 
            qy = float(xform_data[4]) 
            qz = float(xform_data[5])
            qw = float(xform_data[6])
            ...
            # endpoint pose from endpoint tag
            endpoint_data = data[1].split(",")

            tx = (float(endpoint_data[0])) 
            ty = (float(endpoint_data[1])) - 0.13
            tz = (float(endpoint_data[2])) 

            tqx = float(endpoint_data[3]) 
            tqy = float(endpoint_data[4]) 
            tqz = float(endpoint_data[5])
            tqw = float(endpoint_data[6])

            self.endpoint_prim.set_world_pose(
                position=np.array((tx,ty,tz)),
                orientation=np.array((tqx,tqy,tqz,tqw))
            )
