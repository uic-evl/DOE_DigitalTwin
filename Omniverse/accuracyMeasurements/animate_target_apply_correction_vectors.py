...
class TargetControl(BehaviorScript):
    def on_init(self):
        ...
        self.time_stamp = 0.0
        self.time_step = 0
        self.time_delay = 5 # Large delay to account for errors
        self.waiting = False

        # Init last v_endpoint
        self.last_v_endpoint = 0

        # Get path points with corrections from file
        with open("C:/Users/halle/Documents/DigitalTwin/Performance/accuracy/Output/endpoints/avg_correction_mat_8points.txt") as f:
    ...
    def on_play(self):
        self.waiting = False
        self.time_step = 0
        self.filestream = open("C:/Users/halle/Documents/DigitalTwin/Performance/accuracy/Output/for_distance_calc_8points.txt", "w", encoding="utf-8")
    ...
    def on_update(self, current_time: float, delta_time: float):
        ...
        # Every timestep, move to the next point
        if self.waiting is False and self.time_step <= len(self.lines):
            ...
            data = data.split(";")
            voxel = data[0] 
            correction_offset = data[1]

            # Write data BEFORE new command is sent
            #data = self.lines[self.time_step] + "," + str(self.public_distance.Get()) + "," + str(self.ik_status.Get())+ "\n"
            data = voxel + ";" + str(self.v_endpoint.Get()) + ";" + str(self.r_endpoint.Get()) + "\n"
            self.filestream.writelines(data) 

            #logger.warn(self.last_v_endpoint)
            ...
            #logger.warn(str((x,y,z)))
            physicsUtils.set_or_add_translate_op(self.curr_prim, (x,y,z))
            
            # Record v_endpoint
            self.last_v_endpoint = self.v_endpoint.Get()

            # Apply correction vector
            correction_offset_points = correction_offset.split(",")

            dx = float(correction_offset_points[0])
            dy = float(correction_offset_points[1])
            dz = float(correction_offset_points[2])

            physicsUtils.set_or_add_translate_op(self.curr_prim, (x+dx,y+dy,z+dz))
