    ...
    public void ProcessAnimation(UnityEngine.Animations.AnimationStream stream)
    {
        ...
        if (w > 0f)
        {
            ...
            if (!double.IsNaN(theta[1]))
                joints[2].SetLocalRotation(stream, quaternion.Euler(Vector3.up * (float)(theta[1] * w)));
            if (!double.IsNaN(theta[2]))
                joints[3].SetLocalRotation(stream, quaternion.Euler(Vector3.forward * (float)(theta[2] * w)));
            if (!double.IsNaN(theta[3]))
                joints[4].SetLocalRotation(stream, quaternion.Euler(Vector3.up * (float)(theta[3] * w)));
            if (!double.IsNaN(theta[4]))
                joints[5].SetLocalRotation(stream, quaternion.Euler(Vector3.forward * (float)(theta[4] * w)));
            if (!double.IsNaN(theta[5]))
                joints[6].SetLocalRotation(stream, quaternion.Euler(Vector3.up * (float)(theta[5] * w)));
        }
    }
}
